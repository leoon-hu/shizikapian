...
- **发音**（`public/audio/`）由 `npm run audio` 用微软 Edge 朗读接口（Edge-TTS，Azure 神经音色 `zh-CN-XiaoxiaoNeural` / `en-US-JennyNeural`）合成，仅作本应用的离线朗读之用；合成语音的使用条款以微软的为准，不随本仓库的 MIT 许可一并授权。
