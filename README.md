...
- **装到手机上用**：没从主屏幕打开时，首页顶上有一条给家长的「安装到手机」提示（Android 一键安装；iPhone / iPad 画出分享按钮 → 添加到主屏幕；微信里提示去浏览器打开），可关，7 天后再提醒，装好就不再出现。
...
| 离线与安装 | 离线包下载进度 / 是否就绪；Android 一键「安装到主屏幕」，iOS 提示 分享 → 添加到主屏幕（iPhone 在底部、iPad 在右上角）；没有声音时先调媒体音量 |
