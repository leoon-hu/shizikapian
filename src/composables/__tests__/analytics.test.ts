...
const full = { VITE_UMAMI_SCRIPT: 'https://stats.example.com/script.js', VITE_UMAMI_WEBSITE_ID: 'abc-123', VITE_SITE_URL: 'https://kapian.example.com' }

describe('analyticsConfig（需求 4.6：两项都配了才统计）', () => {
  it('两项齐全才有配置，顺手去掉空白，域名取自 VITE_SITE_URL', () => {
    expect(analyticsConfig(full)).toEqual({ script: 'https://stats.example.com/script.js', websiteId: 'abc-123', domain: 'kapian.example.com' })
  ...
  it('没有 VITE_SITE_URL 或它不是合法地址 → 不限域名', () => {
    expect(analyticsConfig({ ...full, VITE_SITE_URL: undefined })?.domain).toBe('')
    expect(analyticsConfig({ ...full, VITE_SITE_URL: 'not a url' })?.domain).toBe('')
  })
})
...
describe('analyticsTag / analyticsAttrs（写进页面的标签）', () => {
  it('defer + src + website id，有域名时带 data-domains', () => {
    ...
    expect(analyticsAttrs(cfg)).toEqual({ defer: true, src: full.VITE_UMAMI_SCRIPT, 'data-website-id': 'abc-123', 'data-domains': 'kapian.example.com' })
    expect(analyticsTag(cfg)).toBe('<script defer src="https://stats.example.com/script.js" data-website-id="abc-123" data-domains="kapian.example.com"></script>')
...
describe('trackBackNavigation（返回键的 popstate 补记一次页面浏览）', () => {
  function fakeWindow(umami?: UmamiTracker) {
    ...
    const win = {
      umami,
      location: { href: 'https://kapian.example.com/#/cards/animals' },
  ...
  it('popstate 后延迟一拍，把当前地址交给 tracker', () => {
    ...
    expect(build({ url: 'https://kapian.example.com/#/quiz', title: 't' })).toEqual({ url: 'https://kapian.example.com/#/cards/animals', title: 't' })
  })
