...
import { shareMessage, shareWay, siteRoot } from '@/composables/share'

describe('分享给朋友：按环境选做法（C6）', () => {
  ...
  it('分享的是站点根地址：去掉 hash 路由与 index.html，子路径部署也对，file:// 退到公开地址', () => {
    expect(siteRoot('https://kapian.jiaci.app/#/cards/animals', 'x')).toBe('https://kapian.jiaci.app/')
    expect(siteRoot('https://example.com/apps/kapian/index.html#/settings', 'x')).toBe('https://example.com/apps/kapian/')
    expect(siteRoot('file:///tmp/dist/index.html', 'https://kapian.jiaci.app')).toBe('https://kapian.jiaci.app')
  })
  it('文案 + 换行 + 链接', () => {
