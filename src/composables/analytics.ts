/**
 * 访问统计（需求 4.6「访问统计」，2026-09-21 四个站统一）：自建的 Umami（开源、无 cookie、不存 IP），页面里只多一行 <script defer>。
 * 脚本地址与站点 id 来自构建期环境变量 VITE_UMAMI_SCRIPT / VITE_UMAMI_WEBSITE_ID（.env，不进仓库）：两项都有才加标签，
 * 别人自己部署时不配就什么都不加。标签由 vite.config.ts 在正式构建时写进 index.html、scripts/seo.ts 写进 cards.html；
 * dev 不加。data-domains 限定只在正式域名（VITE_SITE_URL 的主机名）下上报：本机 preview、双击打开 dist/index.html 都不算。
 * 上报的是页面地址（含 hash 路由）、标题、来源、屏幕尺寸、语言；设置与小测验都不上报。
 */
...
export function analyticsConfig(env: Record<string, string | undefined>): AnalyticsConfig | null {
  ...
  let domain = ''
  try {
    if (env.VITE_SITE_URL?.trim()) domain = new URL(env.VITE_SITE_URL.trim()).hostname
  } catch {
    domain = ''
  }
